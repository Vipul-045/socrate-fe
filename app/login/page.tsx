"use client";

import Link from "next/link";

import { Button } from "@/components/ui/button";
import { MonoLabel } from "@/components/site/primitives";
import { ValueProps } from "@/components/site/value-props";
import { FREE_TIER_NOTE } from "@/lib/content";

const GOOGLE_AUTH_URL = "https://socrate-backend-9eza.onrender.com/auth/google";

function GoogleMark() {
  return (
    <svg className="h-[1.125rem] w-[1.125rem]" viewBox="0 0 24 24" aria-hidden>
      <path
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
        fill="#4285F4"
      />
      <path
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
        fill="#34A853"
      />
      <path
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
        fill="#FBBC05"
      />
      <path
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
        fill="#EA4335"
      />
    </svg>
  );
}

export default function LoginPage() {
  return (
    <main
      id="main"
      className="flex min-h-screen flex-col lg:grid lg:grid-cols-2"
    >
      {/* Left: the pitch. On phones it becomes a compact masthead so the
          sign-in control stays above the fold. */}
      <section className="flex flex-col justify-between bg-surface px-5 pb-10 pt-8 sm:px-8 lg:px-[clamp(2.5rem,5vw,5rem)] lg:py-12">
        <Link
          href="/"
          className="-my-1 inline-block py-1 text-[1.1875rem] font-bold tracking-[-0.02em] text-ink"
        >
          Socrate
        </Link>

        <div className="mt-10 lg:mt-0 lg:max-w-[30rem]">
          <MonoLabel>Learning made easy</MonoLabel>
          <p className="mt-6 text-h2 text-balance">
            Drop your notes. Walk out knowing everything.
          </p>
          <p className="mt-6 max-w-[34ch] text-lede text-ink-soft">
            Upload any document and have an AI tutor explain, summarize, and
            quiz you instantly.
          </p>
        </div>

        <div className="mt-10 hidden lg:block">
          <ValueProps compact />
        </div>
      </section>

      {/* Right: one decision, nothing else on the panel. */}
      <section className="flex flex-1 items-center bg-paper px-5 py-14 sm:px-8 lg:border-l lg:border-line lg:px-[clamp(2.5rem,5vw,5rem)] lg:py-12">
        <div className="mx-auto w-full max-w-[27rem]">
          <h1 className="text-[clamp(1.875rem,3.2vw,2.5rem)] font-medium leading-[1.1] tracking-[-0.026em]">
            Welcome back
          </h1>
          <p className="mt-4 text-lede text-ink-soft">
            Sign in to continue learning where you left off.
          </p>

          <Button
            size="lg"
            block
            variant="outline"
            className="mt-9 h-[3.25rem] gap-3 text-body"
            onClick={() => window.open(GOOGLE_AUTH_URL, "_self")}
          >
            <GoogleMark />
            Continue with Google
          </Button>

          <p className="mt-6 text-small text-ink-faint">{FREE_TIER_NOTE}</p>

          <div className="mt-9 border-t border-line pt-7">
            <p className="text-small text-ink-soft">
              By continuing you agree to our{" "}
              <Link
                href="/terms"
                className="text-ink underline decoration-line-strong underline-offset-4 transition-colors hover:decoration-ink"
              >
                Terms &amp; Conditions
              </Link>{" "}
              and{" "}
              <Link
                href="/privacy"
                className="text-ink underline decoration-line-strong underline-offset-4 transition-colors hover:decoration-ink"
              >
                Privacy Policy
              </Link>
              .
            </p>
            <p className="mt-5 text-small font-medium text-ink">
              New to Socrate?{" "}
              {/* Sign-up is the same Google flow — an account is created on
                  first sign-in. */}
              <button
                type="button"
                onClick={() => window.open(GOOGLE_AUTH_URL, "_self")}
                className="cursor-pointer py-2 font-medium underline decoration-line-strong underline-offset-4 transition-colors hover:decoration-ink"
              >
                Create an account
              </button>
            </p>
          </div>
        </div>
      </section>
    </main>
  );
}
